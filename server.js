...
app.post("/login", async (req, res) => {
  ...
  db.query("SELECT * FROM users WHERE email = ?", [email], async (err, result) => {
    ...
    const token = jwt.sign(
      { id: user.id, role: user.role, supplier_id: user.supplier_id },
...
app.post("/add-request", authenticate, (req, res) => {
  ...
  db.query("SELECT phone FROM users WHERE id = ?", [ngo_id], (err, result) => {
    ...
    db.query(
      "UPDATE food SET requesting_ngoo_phone = ? WHERE food_name = ? AND supplier_id = ?",
      [ngoPhone, food_name, supplier_id],
      (err, updateResult) => {
        if (err) {
          console.error("Database Error:", err);
          return res.status(500).json({ error: "Database error" });
...
app.get("/fetch-requested", authenticate, (req, res) => {
  ...
  const sql = "SELECT requesting_ngoo_phone , food_name , quantity FROM food WHERE supplier_id = ? AND CHAR_LENGTH(CAST(requesting_ngoo_phone AS CHAR)) > 1"
